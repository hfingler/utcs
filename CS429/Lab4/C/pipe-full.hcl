# Caio Lente (ctl684)
# Description of modifications: only the Fetch, Decode and Exetue stages were
# altered in order to allow the simulation to execute the iaddq instruction
# correctly. In short, the program is informed that iaddq is a valid instruction
# (instr_valid), that it requires a regid byte (need_regids) and a constant word
# (need_valC), that its D_rB should be used as the B source (d_srcB) and as the E
# destination (d_dstE), that its ALU input A is E_valC (aluA) and input B is
# E_valB (aluB), that its ALU function is ALUADD (alufun), and that the condition
# codes should be updated (set_cc).
#
#/* $begin pipe-all-hcl */
####################################################################
